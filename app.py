...
login_manager.login_view = 'login' # Redirige a la ruta 'login' si no se ha iniciado sesión

# --- CONFIGURACIÓN DE FLASK-SOCKETIO ---
socketio = SocketIO(app, cors_allowed_origins="*")

# --- HANDLER SOCKETIO PARA UNIRSE A UNA SALA DE MÓDULO ---
@socketio.on('join')
...
# DEFINE LA ZONA HORARIA UNA SOLA VEZ AQUÍ
zona_horaria_chile = pytz.timezone('America/Santiago')

def role_required(role):
    """
    Decorador que restringe el acceso a una ruta a un rol de usuario específico.
    """
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            # Si el usuario no está autenticado o su rol no es el requerido
            if not current_user.is_authenticated or current_user.rol != role:
                flash(f"Acceso denegado. Se requiere el rol de '{role}'.", "error")
                # Lo redirigimos a una página general, como la pantalla pública
...
# --- DEFINICIÓN DEL MODELO (TABLA) DE USUARIO ---
class Usuario(db.Model, UserMixin):
    ...
    # Este campo no se puede leer directamente, es solo para hashear la contraseña
    @property
    def password(self):
        raise AttributeError('password is not a readable attribute')

    # Método para hashear y guardar la contraseña
    @password.setter
...
class Ticket(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    numero_ticket = db.Column(db.String(10), nullable=False, unique=True) # Ej: 'M023'
    rut_cliente = db.Column(db.String(15), nullable=False)
    modulo_solicitado = db.Column(db.String(100), nullable=False)
    estado = db.Column(db.String(20), default='en_espera') # en_espera, en_atencion, finalizado
...
class CrearUsuarioForm(FlaskForm):
    username = StringField('Nombre de Funcionario', validators=[DataRequired()])
    password = PasswordField('Contraseña', validators=[DataRequired()])
    # Creamos un menú desplegable para los roles
    rol = SelectField('Rol', choices=[
        ('staff', 'Staff (Atención)'),
...
class EditarUsuarioForm(FlaskForm):
    username = StringField('Nombre de Funcionario', validators=[DataRequired()])
    # Hacemos la contraseña opcional
